...
In this way, we propose probabilistic programming as a suitable platform not only to
describe and enforce security policies, but also as a mechanical tool to learn
noising and obfuscation functions that satisfy the policies.
...
given some body of data available to him. From a labeled dataset $ D $,
we can employ supervised machine learning to learn a variety of
predictive models that can be candidate blacklist functions.
...
via suppressing particular features Alice's secret information we show that
security and utility policies can be satisfied.

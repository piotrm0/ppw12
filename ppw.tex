...
to religion, Bob can learn predictive models for religion via supervised
...
From {\emph diverse} set of blacklist functions $\blacklists$ and a set of
...
policy $ P(B) $, while $ \exists W \in \whitelists $ satisfies the utility policy $ U(W) $.
In \cite{chakraborty12balancing} we demonstrated one means of learning obfuscation functions
that satisfy security and utility policies, via suppressing and selecting to share only
particular features Alice's secret information.
